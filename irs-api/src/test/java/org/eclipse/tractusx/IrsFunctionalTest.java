...
import org.eclipse.tractusx.controllers.IrsController;
import org.eclipse.tractusx.testing.containers.MinioContainer;
...
class IrsFunctionalTest {
    ...
    void shouldStartJobAndRetrieveResult() {
        ...
        Assertions.assertThat(finishedJob.get().getRelationships()).isNotEmpty();
        assertThat(finishedJob.get().getShells()).isNotEmpty();
        Assertions.assertThat(finishedJob.get().getTombstones()).isNotEmpty();
        Assertions.assertThat(finishedJob.get().getTombstones()).hasSize(3);
